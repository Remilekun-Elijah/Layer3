 ...
 class App {
     static start() {
         ui.getTag('.controls .left').addEventListener('click', e => {
             ux.slideLeft(ui.getTag('.slide'), '100');
             //  console.log(e.target);
         });
         ui.getTag('.controls .right').addEventListener('click', e => {
             ux.slideRight(ui.getTag('.slide'), '100');
             //  console.log(e.target);
         });
         ux.autoslide("lefts");
     }
 ...
 class UI {
     ...
     getTag(tag) {
         let j = document.querySelectorAll(tag);
         if (j.length > 1) return j;
         else return document.querySelector(tag);
 ...
 class UX {
         ...
         if (tag.length > 1) {
             tag.forEach(element => {
                 if (element.style.transform === "translateX(0%)") {
                     element.style.transform = `translateX(-${css}%)`;
                 } else if (element.style.transform === "translateX(-100%)") {
                     element.style.transform = `translateX(-${css+css}%)`;
                 } else if (element.style.transform === "translateX(-200%)") {
                     element.style.transform = `translateX(${css-css}%)`;

                 } else element.style.transform = `translateX(-${css}%)`;
         ...
         if (tag.length > 1) {
             tag.forEach(element => {
                 if (element.style.transform === "") {
                     ...
                     element.style.transform = `translateX(-${css}%)`;
                 } else element.style.transform = `translateX(-${css+css}%`;
     ...
     autoslide(direction) {
         try {
             setInterval(() => {
                 if (direction == "left") this.slideLeft(ui.getTag('.slide'), '100');
                 else if (direction == "right") this.slideRight(ui.getTag(".slide", "100"));
                 else return { success: false };
             }, 3000);
         } catch (err) {
             err.message = "Parameter can only be either 'Left' or 'right'"
 ...
 const ux = new UX();
 //  ux.slideLeft(ui.getTag('.slide', 'translateX(-100%)'));
 App.start();
